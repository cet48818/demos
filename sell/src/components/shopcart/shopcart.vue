...
<script>
  ...
  export default {
    props: {
      selectFoods: {
        type: Array, // 如果type是Array或者Object, default是函数
        default () {
          return []
        }
      },
    ...
    methods: {
      ...
      dropBeforeEnter (el) {
        ...
        while (count--) {
          ...
          if (ball.show) {
            ...
            // x轴和y轴偏移
            let x = rect.left - 32 // 32是小球的默认left值
            // window.innerHeight, 浏览器视口高度; 或document.documentElement.clientHeight
            let y = -(window.innerHeight - rect.top - 22) // 22是小球的默认top值
            // el.style.display = '' // 使元素显示
      ...
      dropEnter (el) {
        ...
        this.$nextTick(() => { // 下一个eventloop里把样式重置过来
...
<style lang="scss">
  ...
  .list-mask {
    ...
    width: 100%;
    height: 100%;
    z-index: 40; // 要小于shopcart的z-index
